// Message storage - can be edited in boss mode
let gentleMessages = {
  warning: [
...
export function generateMockMessage(type, intensity, isActive) {
  const messages = 
    intensity === 'gentle' ? gentleMessages :
    intensity === 'moderate' ? moderateMessages :
    harshMessages;
...
// Boss mode functions to edit messages
export function getMessages(intensity) {
  if (intensity === 'gentle') return gentleMessages;
  if (intensity === 'moderate') return moderateMessages;
  return harshMessages;
}

export function updateMessages(intensity, type, messages) {
  if (intensity === 'gentle') {
    gentleMessages[type] = messages;
  } else if (intensity === 'moderate') {
    moderateMessages[type] = messages;
  } else {
    harshMessages[type] = messages;
  }
  // Save to localStorage
  localStorage.setItem('boss-messages', JSON.stringify({
    gentle: gentleMessages,
    moderate: moderateMessages,
    harsh: harshMessages,
  }));
}

export function loadMessagesFromStorage() {
  const saved = localStorage.getItem('boss-messages');
  if (saved) {
    try {
      const parsed = JSON.parse(saved);
      gentleMessages = parsed.gentle || gentleMessages;
      moderateMessages = parsed.moderate || moderateMessages;
      harshMessages = parsed.harsh || harshMessages;
    } catch (e) {
